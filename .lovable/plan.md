...
8. **Appointment outcomes are silent too.** Rescheduling or cancelling writes a new time to the database and never tells the student.
   - Files: `supabase/functions/record-appointment-outcome/index.ts`.

9. **The Overview tab is nearly empty** — one collapsed panel, no timeline, no documents, no recent messages, no appointment history, even though `case_events` (159 rows) and documents already exist. Team members must hunt across tabs for context.
   - Rebuild Overview as the case summary: status + owner, next action, appointments, timeline, documents, latest messages.
   - Files: `src/components/cases/CaseOverviewPanel.tsx`, `CaseTimeline.tsx`, `src/pages/team/CaseDetailPage.tsx`.

10. **Tab labels and layout change between stages** ("الملف المالي" at one stage, "الملخص المالي" at another; Finance itself contains a second nested tab strip). Stage-dependent chrome makes the page feel like three different screens.
...
- Suggested execution order: items 1–4 (one migration + form fixes, verified with re-queries), then 5–8 (workflow + notifications), then 9–10 (UI restructure), then the cleanup list.
