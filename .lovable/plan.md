...
| Next stage exists but is automated (payment / admin review / enrollment) | "Next stage is <name> — it happens automatically once <payment confirmed / admin review / enrollment payment> is recorded." Name the actual blocked stage rather than listing all of them. |
...
- Title: "Move to <target stage>" instead of the generic "Move to next stage".
- Body: names both ends of the move and the visibility consequence — "This case moves from <current> to <target>. The change is recorded on the case timeline and is visible to the student."
