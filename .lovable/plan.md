...
3. **i18n**
   - Leave the orphaned `intel.recent` key in en/ar locale files (harmless; the parity guard only flags missing keys, not orphans). Remove it only if preferred — default: keep.
