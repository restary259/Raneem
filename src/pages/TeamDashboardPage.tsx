...
const TeamDashboardPage = () => {
  ...
  const handleMarkContacted = async (leadId: string, caseId: string) => {
    ...
    try {
      ...
      pendingRef.current.delete(caseId);
    }
  };
  ...
  const handleDeleteAppointment = async (apptId: string) => {
    ...
    try {
      ...
      if (!error) {
        toast({ title: t("lawyer.appointmentDeleted") });
        try {
          await refetch();
        } catch {}
      ...
      setActionLoadingId(null);
      pendingRef.current.delete(apptId);
  ...
  const renderCaseActions = (c: any, lead: any) => {
    ...
    const REASSIGN_ALLOWED = ["new", "contacted", "appointment_scheduled", "profile_completion", "payment_confirmed"];
    const canReassign = REASSIGN_ALLOWED.includes(status);
    const reassignBtn = canReassign ? (
      <Button
        size="sm"
        variant="ghost"
        className="h-8 text-xs active:scale-95 gap-1 text-muted-foreground"
        onClick={() => setReassignCase(c)}
      >
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button
            ...
            onClick={() => {
              ...
              setScheduleForCase(cs || null);
            }}
          >
            <CalendarDays className="h-3.5 w-3.5" />
            {t("lawyer.makeAppointment")}
          </Button>
          {reassignBtn}
        </div>
      );
    }
    if (["appointment_scheduled"].includes(status)) {
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {linkedAppt && (
            <Button
              ...
              variant="outline"
              className="h-8 text-xs active:scale-95 gap-1"
              onClick={() => setRescheduleAppt(linkedAppt)}
            >
              <CalendarDays className="h-3.5 w-3.5" />
              {t("lawyer.reschedule")}
            </Button>
    ...
    if (["profile_completion", "payment_confirmed"].includes(status)) {
      return (
        <div className="flex gap-2 flex-wrap">
          {phoneBtn}
  ...
  return (
    <div className="min-h-screen bg-background" dir={dir}>
          ...
          <div className="flex items-center justify-between">
              ...
              <div className="min-w-0">
                <h1 className="text-sm sm:text-base font-bold leading-tight truncate">
                  ...
                  <span className="sm:hidden">
                    {t("lawyer.hi", "Hi")}, {profile?.full_name?.split(" ")[0]} 👋
                  </span>
                </h1>
                <p className="hidden sm:block text-xs text-white/70 truncate">
                  ...
                  {lastRefreshedAt && (
                    <span className="ms-2 text-white/40">
                      · {t("common.lastRefreshed", "Updated")}{" "}
                      {Math.round((Date.now() - lastRefreshedAt.getTime()) / 1000)}s
                    </span>
                  )}
                </p>
              </div>
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              <Button
                variant="ghost"
                size="sm"
                className="text-white/70 hover:text-white hover:bg-white/10 active:scale-95"
                onClick={() => navigate("/")}
              >
                        ...
                        <button
                          ...
                          className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-all active:scale-95 border ${
                            active
                              ...
                              : `bg-muted text-muted-foreground hover:bg-muted/80 ${NEON_BORDERS[f] || "border-transparent"}`
                  ...
                  <div className="space-y-3">
                    ...
                    {filteredCases.map((c) => {
                      ...
                      return (
                        <Card
                          key={c.id}
                          className={`transition-all duration-300 border-2 ${neonBorder} ${sla ? "ring-1 ring-destructive/30" : ""}`}
                        >
                          <CardContent className="p-3 space-y-2">
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <span
                                className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold shrink-0 ${statusColor}`}
                              >
                                {String(statusLabel)}
                              </span>
                            </div>
                            {isPaid && (
                              <div className="grid grid-cols-2 gap-2 text-xs">
                                <div className="p-2 bg-emerald-50 rounded">
                                  <span className="text-muted-foreground">{t("lawyer.serviceFee")}</span>
                                  <p className="font-semibold">{c.service_fee} ₪</p>
                                </div>
                                <div className="p-2 bg-emerald-50 rounded">
                                  <span className="text-muted-foreground">{t("lawyer.yourCommission")}</span>
                                  <p className="font-semibold">{c.lawyer_commission} ₪</p>
                                </div>
                              </div>
                            )}
                            {c.notes && (
                              <p className="text-xs text-muted-foreground bg-muted/30 p-2 rounded">{c.notes}</p>
                            )}
                            {c.admin_notes && (
                      ...
                      <Card>
                        <CardContent className="p-8 text-center">
                          ...
                          <p className="text-xs text-muted-foreground/60 mt-1">
                            {t("lawyer.noCasesHint", "Cases assigned to you will appear here.")}
                          </p>
                        </CardContent>
                      </Card>
                    )}
                              ...
                              <div className="flex items-start justify-between gap-2">
                                <div className="flex items-center gap-3 min-w-0">
                                  <div className="text-center shrink-0 bg-purple-50 rounded-lg px-2.5 py-1">
                                    <p className="text-lg font-bold leading-tight text-purple-700">
                                  ...
                                  <div className="min-w-0">
                                    ...
                                    <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                                      {linkedCase && (
                                        <span
                                          className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold ${statusColor}`}
                                        >
                                          {String(statusLabel)}
                                        </span>
                                      )}
                                      {linkedLead && (linkedLead as any).source_type && (
                                        <Badge variant="outline" className="text-[10px]">
                                          {String(
                                            t(
                                              `lawyer.sources.${(linkedLead as any).source_type}`,
                                              (linkedLead as any).source_type,
                                            ),
                                          )}
                                        </Badge>
                                      )}
                              ...
                              <div className="flex gap-2 flex-wrap pt-1 border-t border-muted/50">
                                ...
                                {isApptStage && linkedCase && (
                                  <Button
                                    size="sm"
                                    className="h-8 text-xs gap-1"
                                    onClick={() => openProfileModal(linkedCase)}
                                  >
                                    <FileText className="h-3.5 w-3.5" />
                                    {t("lawyer.completeProfile")}
                                  </Button>
      ...
      <ProfileCompletionModal
        ...
        leads={leads}
        userId={user?.id}
        onClose={() => setProfileCase(null)}
        onCompleted={(f) => setCaseFilter(f as CaseFilterTab)}
        refetch={refetch}
      />
      <ScheduleDialog
        scheduleForCase={scheduleForCase}
        leads={leads}
        userId={user?.id}
        onClose={() => setScheduleForCase(null)}
        refetch={refetch}
      />
      ...
      <PaymentConfirmDialog
        caseId={paymentConfirm}
        saving={saving}
        onConfirm={confirmPaymentAndSubmit}
      ...
      {isMobile && (
        <nav
          className="fixed bottom-0 inset-x-0 z-50 bg-white border-t border-gray-200 px-2 py-2 lg:hidden"
