            ...
            <div className="grid h-full md:grid-cols-[320px_minmax(0,1fr)] xl:grid-cols-[320px_minmax(430px,1fr)_340px]">
              ...
              <main className={cn("flex min-h-0 flex-col", mobile && !selectedId && "hidden")}>
                {!active ? <EmptyState title={t("empty.select")} icon={MessageCircle} className="h-full" /> : <>
                  ...
                  <div className="border-t p-3"><Textarea value={composer} onChange={(e) => setComposer(e.target.value)} placeholder={t("conversation.composer")} className="min-h-20 resize-none" /><div className="mt-2 flex items-center justify-between gap-2"><p className="max-w-xl text-[11px] text-muted-foreground">{requiresApprovedTemplate(active.last_inbound_at) ? t("conversation.windowClosed") : t("conversation.sendUnavailable")}</p><Button disabled><MessageCircle className="me-2 h-4 w-4" />WhatsApp</Button></div></div>
                </>}
              </main>
