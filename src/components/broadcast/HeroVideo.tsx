  ...
  return (
    <div className="relative w-full h-[60vh] md:h-[70vh] text-white overflow-hidden">
      {post.videoUrl ? (
        ...
        <img src={post.posterUrl} alt={post.title} className="absolute top-0 left-0 w-full h-full object-cover" />
      )}
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent" />
      <div className="relative z-10 flex flex-col justify-end h-full p-6 md:p-12 text-right">
        <h1 className="text-3xl md:text-5xl font-extrabold mb-2 text-shadow-lg animate-fade-in">{post.title}</h1>
        <p className="max-w-xl text-lg mb-6 text-shadow animate-fade-in animation-delay-300">{post.description}</p>
