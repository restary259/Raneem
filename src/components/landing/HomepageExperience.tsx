...
const HomepageExperience = () => {
  ...
  return (
    <div className="homepage-experience bg-background">
      ...
      <section className="relative overflow-hidden border-t border-border bg-background text-primary"><BrandArch className="opacity-10" /><div className="container relative z-10 grid gap-7 py-14 sm:py-18 lg:grid-cols-[1fr_auto] lg:items-end"><div className="max-w-3xl"><p className="text-xs font-bold uppercase tracking-[0.18em] text-brand">{t("homepage.final.eyebrow")}</p><h2 className="mt-3 text-balance font-editorial text-3xl leading-tight sm:text-5xl">{t("homepage.final.title")}</h2><p className="mt-4 max-w-2xl leading-7 text-muted-foreground">{t("homepage.final.body")}</p></div><div className="flex flex-col gap-3 sm:flex-row lg:flex-col"><Button asChild size="lg" variant="cta"><Link to="/apply">{t("homepage.actions.startAssessment")}<Arrow /></Link></Button><Button asChild size="lg" variant="outline" className="rounded-full"><a href={whatsappBusinessUrl("مرحبا، بدي أعرف شو خطوتي الجاية للدراسة بألمانيا.")} target="_blank" rel="noopener noreferrer"><MessageCircle />{t("homepage.actions.whatsapp")}</a></Button></div></div><span aria-hidden="true" className="darb-spectrum darb-spectrum-lg block rounded-none" /></section>
    </div>
  );
};
