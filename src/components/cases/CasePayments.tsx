...
interface Props {
  ...
  /**
   * Retained for compatibility with CaseFinance.
   *
   * Payment amounts are no longer entered manually.
   */
  canManage: boolean;

  /**
   * Existing admin-only payment-history confirmation.
   *
   * This remains separate from the new DARB finance confirmation.
   */
  canConfirm?: boolean;
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm font-semibold">{t("finance.payments.title")}</p>
      </div>

      {payments.length === 0 ? (
        <div className="rounded-md border border-dashed p-4">
          <p className="text-sm text-muted-foreground">{t("finance.payments.empty")}</p>
        </div>
      ) : (
              ...
              <div key={payment.id} className="space-y-2 rounded-md border p-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-sm font-medium">{formatILS(Number(payment.amount || 0))}</p>
      ...
      <p className="text-xs text-muted-foreground">
        DARB service fees are now confirmed from the calculated service total above. Payment amounts cannot be entered
