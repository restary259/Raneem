...
const AppointmentCalendar = ({ userId, cases, leads }: AppointmentCalendarProps) => {
  ...
  const getLeadName = (leadId: string) => leads.find((l: any) => l.id === leadId)?.full_name || 'غير معروف';
  ...
  const handleCreate = async () => {
    ...
    if (error) {
      ...
      // Auto-update case status to 'appointment' if linked
      if (newAppt.case_id) {
  ...
  const dayNames = ['اثنين', 'ثلاثاء', 'أربعاء', 'خميس', 'جمعة', 'سبت', 'أحد'];

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-bold text-base flex items-center gap-2">
          <Calendar className="h-4 w-4" /> التقويم والمواعيد
        </h2>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-md">
            ...
            <div className="space-y-3">
              <div>
                <Label className="text-xs">ربط بملف (اختياري)</Label>
                <Select value={newAppt.case_id} onValueChange={handleCaseSelect}>
                  <SelectTrigger><SelectValue placeholder="اختر ملف طالب..." /></SelectTrigger>
                  <SelectContent>
                    {cases.map((c: any) => (
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <Input value={newAppt.location} onChange={e => setNewAppt(p => ({ ...p, location: e.target.value }))} placeholder="مكتب / أونلاين" />
                </div>
              </div>
              <div>
              ...
              <Button onClick={handleCreate} disabled={saving} className="w-full">{saving ? 'جاري الحفظ...' : 'إضافة الموعد'}</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
      ...
      <Card>
        <CardContent className="p-3">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="font-bold text-sm">{format(currentMonth, 'MMMM yyyy', { locale: ar })}</span>
          ...
          <div className="grid grid-cols-7 gap-0.5 mb-1">
            {dayNames.map(d => <div key={d} className="text-center text-[10px] text-muted-foreground font-medium py-1">{d}</div>)}
          </div>
      ...
      {selectedDate && (
        <div className="space-y-2">
          <h3 className="text-sm font-semibold">
            مواعيد {format(selectedDate, 'd MMMM yyyy', { locale: ar })}
          </h3>
          {selectedDayAppts.length === 0 ? (
            <p className="text-xs text-muted-foreground py-4 text-center">لا توجد مواعيد في هذا اليوم</p>
          ) : (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-3 text-xs text-muted-foreground">
                        ...
                        <span>{appt.duration_minutes} دقيقة</span>
      ...
      {!selectedDate && appointments.filter(a => new Date(a.scheduled_at) >= new Date()).length > 0 && (
        <div className="space-y-2">
          <h3 className="text-sm font-semibold">المواعيد القادمة</h3>
          {appointments
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {format(new Date(appt.scheduled_at), 'd MMM - HH:mm', { locale: ar })}
                        {appt.location && ` • ${appt.location}`}
                      </p>
                    </div>
                    <Badge variant="outline" className="text-[10px]">{appt.status === 'scheduled' ? 'مجدول' : appt.status}</Badge>
                  </div>
                </CardContent>
              </Card>
            ))}
