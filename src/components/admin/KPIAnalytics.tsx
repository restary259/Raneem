    ...
    <div className="space-y-6">
      <Card className={`overflow-hidden ${totalProfit >= 0 ? 'bg-gradient-to-br from-emerald-500 to-emerald-700' : 'bg-gradient-to-br from-red-500 to-red-700'} text-white border-0`}>
        <CardContent className="p-6 text-center">
          ...
          <p className="text-sm opacity-80">{t('kpi.netProfit')}</p>
          <p className="text-4xl font-bold mt-1">{totalProfit} €</p>
          <p className="text-xs opacity-70 mt-2">{t('kpi.paidStudents', { count: paidCases.length })}</p>
        </CardContent>
      </Card>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <Card className="border-emerald-200 bg-emerald-50/50"><CardContent className="p-4 text-center"><p className="text-xs text-emerald-700 font-medium">{t('kpi.revenue')}</p><p className="text-2xl font-bold text-emerald-700">{totalRevenue} €</p></CardContent></Card>
        <Card className="border-red-200 bg-red-50/50"><CardContent className="p-4 text-center"><p className="text-xs text-red-700 font-medium">{t('kpi.expenses')}</p><p className="text-2xl font-bold text-red-700">{totalCosts} €</p></CardContent></Card>
        <Card><CardContent className="p-4 text-center"><p className="text-xs text-muted-foreground">{t('kpi.profitPerStudent')}</p><p className="text-2xl font-bold">{avgProfitPerStudent} €</p></CardContent></Card>
      </div>
      ...
      {influencerMetrics.length > 0 && (
        <div>
          <h2 className="text-lg font-bold mb-3 flex items-center gap-2"><TrendingUp className="h-5 w-5" />{t('kpi.agentPerformance')}</h2>
          <div className="grid gap-3">
            {influencerMetrics.map(i => (
              <Card key={i.id}><CardContent className="p-4">
                <h3 className="font-semibold mb-2">{i.full_name}</h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-sm">
                  <div className="text-center p-2 bg-muted/40 rounded"><p className="text-xs text-muted-foreground">{t('kpi.clients')}</p><p className="font-bold">{i.totalLeads}</p></div>
                  <div className="text-center p-2 bg-muted/40 rounded"><p className="text-xs text-muted-foreground">{t('kpi.quality')}</p><p className="font-bold">{i.qualityRate}%</p></div>
                  <div className="text-center p-2 bg-muted/40 rounded"><p className="text-xs text-muted-foreground">{t('kpi.conversion')}</p><p className="font-bold">{i.convRate}%</p></div>
                  <div className="text-center p-2 bg-muted/40 rounded"><p className="text-xs text-muted-foreground">{t('kpi.costPerStudent')}</p><p className="font-bold">{i.costPerPaid} €</p></div>
                </div>
              </CardContent></Card>
            ))}
          </div>
