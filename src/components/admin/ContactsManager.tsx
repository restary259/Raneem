...
const ContactsManager: React.FC<ContactsManagerProps> = ({ contacts, onRefresh }) => {
  ...
  const filtered = contacts.filter(c =>
    !search || c.data?.name?.toLowerCase().includes(search.toLowerCase()) || c.data?.email?.toLowerCase().includes(search.toLowerCase())
  );
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <Input className="ps-10" placeholder="بحث في الرسائل..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
          ...
          <Card key={c.id} className={c.status === 'new' ? 'border-amber-300 bg-amber-50/30' : ''}>
            <CardContent className="pt-4">
              <div className="flex flex-wrap justify-between items-start gap-2 mb-2">
                <div>
                  <h3 className="font-bold">{c.data?.name || 'بدون اسم'}</h3>
                  <p className="text-sm text-muted-foreground">{c.data?.email} • {c.data?.whatsapp || '—'}</p>
                </div>
              ...
              <div className="mt-3 flex gap-2">
                {c.status === 'new' && <Button size="sm" onClick={() => updateStatus(c.id, 'replied')}>تم الرد</Button>}
                {c.status !== 'archived' && <Button size="sm" variant="outline" onClick={() => updateStatus(c.id, 'archived')}>أرشفة</Button>}
              </div>
            </CardContent>
          </Card>
        ))}
        {filtered.length === 0 && <p className="p-8 text-center text-muted-foreground">لا توجد رسائل</p>}
      </div>
    </div>
  );
};
