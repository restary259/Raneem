...
const SecurityPanel: React.FC<SecurityPanelProps> = ({ loginAttempts }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">{t('admin.security.recentLogins')}</CardTitle></CardHeader>
        <CardContent>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b bg-muted/50"><th className="px-3 py-2 text-start">{t('admin.security.email')}</th><th className="px-3 py-2 text-start">{t('admin.security.result')}</th><th className="px-3 py-2 text-start">{t('admin.security.date')}</th></tr></thead>
              <tbody>
