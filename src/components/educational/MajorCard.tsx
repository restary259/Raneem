...
const MajorCard = ({ major, onMajorClick, searchQuery }: MajorCardProps) => {
  ...
  return (
    <Button type="button" variant="ghost" className={`group relative isolate flex h-full min-h-52 w-full flex-col items-stretch overflow-hidden rounded-lg border border-story-foreground/15 p-0 text-start text-story-foreground whitespace-normal shadow-[inset_0_1px_0_hsl(var(--story-foreground)/0.3),0_5px_0_hsl(var(--foreground)/0.18),0_14px_26px_-12px_hsl(var(--foreground)/0.35)] transition-[transform,box-shadow] duration-300 hover:-translate-y-1 hover:text-story-foreground hover:shadow-[inset_0_1px_0_hsl(var(--story-foreground)/0.4),0_8px_0_hsl(var(--foreground)/0.18),0_20px_32px_-12px_hsl(var(--foreground)/0.4)] focus-visible:ring-2 focus-visible:ring-ring active:translate-y-0 motion-reduce:transform-none motion-reduce:transition-none ${categorySurfaces[major.categoryId] ?? 'bg-story-sky hover:bg-story-sky'}`} onClick={() => onMajorClick(major)}>
      ...
      <span className="relative flex h-full w-full flex-col p-5 sm:p-6">
        <span className="mb-5 flex w-full items-start justify-between gap-3">
          <span className="grid size-12 shrink-0 place-items-center rounded-lg border border-story-foreground/20 bg-story-foreground/10 shadow-[inset_0_1px_1px_hsl(var(--story-foreground)/0.18),0_5px_12px_hsl(var(--foreground)/0.22)]"><BookOpen className="size-5" /></span>
        ...
        <span className="mt-auto flex w-full items-center justify-center gap-2 rounded-lg border border-story-foreground/20 bg-story-foreground/15 px-4 py-3 text-sm font-bold text-story-foreground shadow-[inset_0_1px_0_hsl(var(--story-foreground)/0.25),0_4px_0_hsl(var(--foreground)/0.2)] transition-transform duration-200 group-active:translate-y-0.5 motion-reduce:transition-none">
