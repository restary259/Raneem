...
const NAV_CONFIG: Record<AppRole, NavItem[]> = {
  ...
  team_member: [
    ...
    { key: 'nav.todayAppts', icon: CalendarDays, href: '/team/appointments/today' },
      ...
      <SidebarMenu className="mt-2 px-2">
        {items.map((item) => {
          const isActive = location.pathname === item.href ||
            (item.href !== '/admin' && item.href !== '/team' && item.href !== '/partner' && item.href !== '/student/checklist' && location.pathname.startsWith(item.href));
          return (
