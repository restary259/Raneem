...
const UniversityCarousel: React.FC<UniversityCarouselProps> = ({ universities, activeCountry, expandedCard, setExpandedCard }) => {
  const { t } = useTranslation();

  if (universities.length === 0) {
    return (<div className="text-center py-12"><p className="text-gray-500">{t('partners.comingSoon')}</p></div>);
  }

  return (
    <div className="space-y-6">
      <h3 className="text-2xl font-bold text-primary flex items-center gap-2">
        <GraduationCap className="h-6 w-6" />{t('partners.universities')}
      </h3>
            ...
            {universities.map((university, index) => (
              <CarouselItem key={index} className="pl-2 md:pl-4 basis-full sm:basis-1/2 lg:basis-1/3 xl:basis-1/4">
                <div className="h-full">
                  <InteractiveCard id={`uni-${activeCountry}-${index}`} isExpanded={expandedCard === `uni-${activeCountry}-${index}`} onToggle={setExpandedCard}>
                      ...
                      <div className="flex flex-col items-center text-center space-y-4 flex-grow">
                        ...
                        <div className="space-y-2 flex-grow">
                          ...
                          <p className="text-xs md:text-sm text-muted-foreground flex items-center gap-1 justify-center">
                            <MapPin className="h-3 w-3 md:h-4 md:w-4 flex-shrink-0" /><span className="line-clamp-1">{university.location}</span>
                          </p>
                        </div>
                        {expandedCard === `uni-${activeCountry}-${index}` && (
                          <div className="space-y-4 animate-fade-in w-full">
                            ...
                            <div className="space-y-2">
                              <h5 className="font-semibold text-primary text-sm">{t('partners.keyFacts')}</h5>
                              <div className="flex flex-wrap gap-1 justify-center">
                                {university.keyFacts.map((fact, factIndex) => (<Badge key={factIndex} variant="outline" className="text-xs">{fact}</Badge>))}
                              </div>
                            </div>
                            <Button asChild className="w-full" size="sm">
                              <a href={university.websiteUrl} target="_blank" rel="noopener noreferrer">
                                {t('partners.visitWebsite')} <ExternalLink className="h-3 w-3 ms-2" />
                              </a>
                            </Button>
                          </div>
                        )}
