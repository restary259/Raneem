...
- Frontend: `TeamAnalyticsPage` → `.rpc("get_my_cash_debts")` (no `.eq` — scoped server-side); `MemberDetailDrawer` → `.rpc("get_member_cash_debts", { p_member_id })` then filter `debt_status === "pending"` client-side (matches the previous `.eq("debt_status","pending")`). `types.ts` gained both RPC signatures.
