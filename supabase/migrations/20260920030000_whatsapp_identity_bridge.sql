-- WhatsApp identity bridge: connect the WhatsApp channel to the existing DARB person graph.
-- Safe rules:
--   1. Inbound resolution is server-side and idempotent.
--   2. Exact canonical phone matches may auto-link only when unambiguous.
--   3. Ambiguous/conflicting matches remain unlinked for staff review.
--   4. Unknown WhatsApp contacts remain lightweight WhatsApp leads.
--   5. This migration never creates a core lead/case.
...
BEGIN
  ...
  -- DARB's primary audience commonly stores Israeli mobile numbers locally
  -- (05XXXXXXXX), while WhatsApp normally supplies +9725XXXXXXXX.
...
BEGIN
  ...
  -- Never override an already-confirmed/stable identity.
  IF EXISTS (
    SELECT 1
    FROM public.whatsapp_leads
    WHERE id = p_whatsapp_lead_id
  ...
  -- Profiles that are not attached to the one matching case remain a
  -- conflicting identity signal and prevent automatic linking.
  IF v_case_count = 1 THEN
    SELECT count(*)
      INTO v_unattached_profile_count
    FROM public.profiles p
    WHERE p.deleted_at IS NULL
      AND public.whatsapp_identity_phone_key(COALESCE(p.phone_number, '')) = v_key
      ...
      AND NOT EXISTS (
        ...
        WHERE c2.id = v_case_id
          AND c2.student_user_id = p.id
      );
  ELSE
  ...
  -- More than one case, or a conflicting profile, is ambiguous. Do not
  -- silently choose a person. Multiple legacy leads are also left for review.
  IF v_case_count > 1 OR v_unattached_profile_count > 0 OR v_lead_count > 1 OR v_profile_count > 1 AND v_case_count = 0 THEN
  ...
  -- No case exists. A single exact profile or single exact lead is still a
  -- usable identity anchor. If both exist, link both because they share the
  -- same exact canonical phone and there is no competing case.
...
-- Replace the old last-9-digit matcher with canonical-phone matching.
...
BEGIN
  ...
  SELECT public.whatsapp_identity_phone_key(w.whatsapp_number)
    INTO v_key
  ...
  RETURN QUERY
  SELECT 'case'::text, c.id, c.full_name,
         COALESCE(c.case_reference, c.status),
  ...
  RETURN QUERY
  SELECT 'profile'::text, p.id, p.full_name,
         COALESCE(p.email, ''),
         NULL::uuid, p.id, NULL::uuid
  FROM public.profiles p
  WHERE p.deleted_at IS NULL
    AND public.whatsapp_identity_phone_key(COALESCE(p.phone_number, '')) = v_key
  LIMIT 10;
...
-- Staff links must themselves be phone-consistent. This prevents a UI or
-- compromised client from combining unrelated lead/case/profile IDs.
...
BEGIN
  ...
  SELECT whatsapp_number, linked_case_id
    INTO v_number, v_prev_case
  FROM public.whatsapp_leads
  WHERE id = p_whatsapp_lead_id
  FOR UPDATE;
  ...
  IF p_lead_id IS NOT NULL AND NOT EXISTS (
    ...
    RAISE EXCEPTION 'Selected lead does not match the WhatsApp phone';
  END IF;
  ...
  IF p_case_id IS NOT NULL AND NOT EXISTS (
    ...
    RAISE EXCEPTION 'Selected case does not match the WhatsApp phone';
  END IF;
  ...
  IF p_profile_id IS NOT NULL AND NOT EXISTS (
    ...
    RAISE EXCEPTION 'Selected profile does not match the WhatsApp phone';
  END IF;
  ...
  IF v_target_count = 0 THEN
    RAISE EXCEPTION 'At least one matching identity target is required';
  END IF;
...
-- Hook the resolver into the existing idempotent inbound path.
-- The resolver is called only after the WhatsApp contact/conversation exists.
  ...
  LOOP
    ...
    IF v_type = 'whatsapp.message' THEN
      ...
      v_results := v_results || jsonb_build_object(
        'event_type', v_type,
        'result', CASE WHEN v_message_id IS NULL THEN 'duplicate' ELSE 'stored' END,
        'identity', coalesce(v_resolution, jsonb_build_object('status', 'not_reprocessed'))
      );
    ...
    ELSIF v_type IN ('whatsapp.status', 'whatsapp.message_error') THEN
      ...
      IF public.whatsapp_status_rank(v_status) > v_existing_rank
         OR (public.whatsapp_status_rank(v_status) = v_existing_rank AND v_occurred > v_existing_at) THEN
        UPDATE public.whatsapp_messages
        SET delivery_status = v_status,
            status_updated_at = v_occurred,
            error_status = CASE WHEN v_status = 'failed' THEN nullif(v_event->>'error_code', '')::integer ELSE error_status END,
            error_message = CASE WHEN v_status = 'failed' THEN nullif(v_event->>'error_message', '') ELSE error_message END
        WHERE provider_message_id = v_provider_id;
      END IF;

      v_results := v_results || jsonb_build_object('event_type', v_type, 'result', 'updated');
    ELSE
      INSERT INTO public.whatsapp_ingest_log (delivery_id, event_type, payload)
