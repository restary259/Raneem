-- ════════════════════════════════════════════════════════════════════════
-- Agent direct-invite permission flag.
--
-- Admin-controlled `profiles.agent_can_invite_directly`. When true, the agent
-- may send partner/ambassador email invitations and create those accounts
-- directly from their own dashboard (agent-invite-recruit edge function).
--
-- The flag is ADMIN-ONLY settable, exactly like is_master_partner /
-- is_manager / agent_id: restrict_profiles_write() forces it to false on
-- INSERT and raises on any non-admin UPDATE.
-- ════════════════════════════════════════════════════════════════════════

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS agent_can_invite_directly boolean NOT NULL DEFAULT false;

-- ────────────────────────────────────────────────────────────────────────
-- Recreate restrict_profiles_write() — identical to the live definition from
-- 20260814140100, plus agent_can_invite_directly (INSERT force-false, UPDATE
-- admin-only guard).
-- ────────────────────────────────────────────────────────────────────────
