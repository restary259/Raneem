DROP FUNCTION IF EXISTS public.get_members_directory(app_role);

CREATE OR REPLACE FUNCTION public.get_members_directory(p_role app_role DEFAULT NULL::app_role)
 RETURNS TABLE(requester_id uuid, full_name text, email text, phone_number text, city text, created_at timestamp with time zone, role text, referral_code text, agent_id uuid, is_deactivated boolean, assigned_cases bigint, enrolled_cases bigint, team_reward_total numeric, recruited_count bigint, earned_override numeric, students_count bigint, earned_referral numeric, total_earned numeric, paid_amount numeric, locked_amount numeric, available_amount numeric, open_requests bigint, open_request_amount numeric, last_request_at timestamp with time zone, direct_enrolled_cases bigint, network_enrolled_cases bigint)
 LANGUAGE sql
...
AS $function$
  ...
  -- Cases the account referred personally (self-referral / own apply link).
  direct_enrolled AS (
    SELECT c.partner_id AS pid, count(*) AS enrolled
    FROM public.cases c
  ...
  -- Cases referred by the partners/ambassadors an agent recruited.
  network_enrolled AS (
    SELECT rec.agent_id AS aid, count(*) AS enrolled
    FROM public.cases c
    JOIN public.profiles rec ON rec.id = c.partner_id
    WHERE rec.agent_id IS NOT NULL
      AND rec.deleted_at IS NULL
      AND c.deleted_at IS NULL
      AND c.status = 'enrollment_paid'
    GROUP BY rec.agent_id
  ),
  partner_cases AS (
  ...
  SELECT b.id,
         ...
         coalesce(de.enrolled, 0) AS direct_enrolled_cases,
         coalesce(ne.enrolled, 0) AS network_enrolled_cases
  FROM base b
  ...
  LEFT JOIN direct_enrolled de ON de.pid = b.id
  LEFT JOIN network_enrolled ne ON ne.aid = b.id
