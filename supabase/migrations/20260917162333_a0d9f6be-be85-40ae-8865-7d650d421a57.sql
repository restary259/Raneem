ALTER TABLE public.cases
  ADD COLUMN IF NOT EXISTS intel_student_intake jsonb,
  ADD COLUMN IF NOT EXISTS intel_intake_updated_at timestamptz,
  ADD COLUMN IF NOT EXISTS intel_intake_updated_by uuid REFERENCES public.profiles(id) ON DELETE SET NULL;

COMMENT ON COLUMN public.cases.intel_student_intake IS 'Validated case-level StudentIntake snapshot used by DARB Major Intelligence. The raw degree_interest field is never overwritten.';
COMMENT ON COLUMN public.cases.intel_intake_updated_at IS 'Time the Major Intelligence student intake was last confirmed.';
COMMENT ON COLUMN public.cases.intel_intake_updated_by IS 'Staff member who last confirmed the Major Intelligence student intake.';
...
BEGIN
  ...
  LOOP
    IF p_intake ? v_key AND jsonb_typeof(p_intake -> v_key) <> 'null' THEN
      ...
      END IF;
      IF v_key IN ('mathUnits','englishUnits','furtherUnits') AND v_number <> trunc(v_number) THEN
  ...
  SET intel_student_intake = v_clean,
      ...
      english_level = CASE WHEN v_clean ? 'englishGrade' THEN (v_clean ->> 'englishGrade') ELSE english_level END,
      updated_at = now()
  WHERE id = p_case_id;
