...
BEGIN
  ...
  ELSE
    -- The invoice number mirrors the case number (one invoice per case), so
    -- staff and students never have to reconcile two different identifiers.
    IF NULLIF(btrim(coalesce(v_case.case_reference, '')), '') IS NOT NULL THEN
      v_number := regexp_replace(btrim(v_case.case_reference), '^DRB-', 'DRB-INV-');
      IF v_number = btrim(v_case.case_reference) THEN
        v_number := 'DRB-INV-' || btrim(v_case.case_reference);
      END IF;
    ELSE
