-- Close the student direct-DM back-channel opened by 20260817060000.
--
-- request_payout_via_chat now creates a real student→admin direct thread so the
-- payout request lands in the admin's direct-message inbox. send_direct_message
-- only checks thread membership (no staff gate), so a student with a payout
-- thread could post ARBITRARY messages to the admin there — bypassing the
-- "students message only via the case thread" rule.
--
-- Fix: students may only send_direct_message into threads linked to one of
-- their own payout requests. General chat stays case-thread-only; the payout
-- conversation remains fully usable — request_payout_via_chat inserts the
-- payout_requests row BEFORE calling send_direct_message, so the gate passes.
-- Body is byte-for-byte identical to 20260814182120 otherwise.
