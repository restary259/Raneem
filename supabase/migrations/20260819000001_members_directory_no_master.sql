-- ═══════════════════════════════════════════════════════════════════════
-- Drop the Master Partner columns from get_members_directory so the RPC
-- matches the de-mastered frontend (Master Partner was removed from the
-- active app + commission engine on 2026-08-17; see
-- 20260818000000_commission_simplification.sql). The previous definition
-- (20260819000000_members_directory_rpc.sql) still selected
-- is_master_partner / master_partner_id / earned_master_override and joined
-- a master_network CTE that the frontend no longer consumes.
--
-- Idempotent: DROP FUNCTION IF EXISTS then CREATE OR REPLACE.
-- ═══════════════════════════════════════════════════════════════════════

DROP FUNCTION IF EXISTS public.get_members_directory(public.app_role);
...
REVOKE ALL ON FUNCTION public.get_members_directory(public.app_role) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_members_directory(public.app_role) TO authenticated;
