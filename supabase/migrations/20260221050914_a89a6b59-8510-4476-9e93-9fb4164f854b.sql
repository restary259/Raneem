...
-- 1. Add column to track who added translation service
ALTER TABLE public.student_cases
ADD COLUMN IF NOT EXISTS translation_added_by_user_id uuid DEFAULT NULL;

-- 2. Update auto_split_payment trigger to include translation reward
...
BEGIN
  ...
  IF NEW.case_status = 'paid' AND (OLD.case_status IS DISTINCT FROM 'paid') THEN
    ...
    -- Influencer reward — status 'pending'
