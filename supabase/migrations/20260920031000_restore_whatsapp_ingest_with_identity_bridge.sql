-- Restore the canonical WhatsApp ingest implementation and add the identity bridge
-- without dropping template-status handling or changing the existing ingest contract.
  ...
  LOOP
    ...
    IF v_type = 'whatsapp.message' THEN
      ...
      IF v_message_id IS NOT NULL THEN
        ...
        -- Resolve the WhatsApp contact into the existing DARB identity graph
        -- only when the canonical phone match is unambiguous.
        PERFORM public.whatsapp_auto_resolve_identity(v_lead_id);
      END IF;
