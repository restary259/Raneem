...
// These failures need a code/payload/configuration fix, not another send attempt.
  ...
  for (const queue of ["auth_emails", "transactional_emails"]) {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      try {
        ...
        // Permanent 4xx/payload errors will not succeed on retry.
        // Move them directly to the DLQ instead of creating repeated failed attempts.
        if (isPermanentFailure(error)) {
          await moveToDlq(supabase, queue, msg, `Permanent email failure: ${errorMsg.slice(0, 1000)}`);
