...
// and the project's anon key is one — it is shipped in the browser bundle.
// Sending mail from our own domain to an arbitrary recipient therefore has to
// be gated in-function: the service role (other functions, cron) or a staff
// user JWT.
...
Deno.serve(async (req) => {
  ...
  const category: "transactional" | "marketing" =
    (requestedCategory ?? (template as { category?: string }).category ?? "transactional") === "marketing"
      ? "marketing"
      : "transactional";
  ...
  const suppressed = (suppressionRows ?? []).some(
    (row: { scope?: string | null }) => (row.scope ?? "all") === "all" || category === "marketing",
  );
  ...
  const { error: enqueueError } = await supabase.rpc("enqueue_email", {
    ...
    payload: {
      ...
      // Lovable's email API requires an unsubscribe_token for transactional
      // sends as well. The token is stable per recipient and is already
      // created/reused above.
