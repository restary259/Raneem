      ...
      const emailRes = await fetch(
        `${Deno.env.get("SUPABASE_URL")}/functions/v1/send-branded-email`,
        {
          ...
          body: JSON.stringify({
            ...
            html: `<h2>Welcome to Darb, ${full_name}!</h2><p>Your account has been created.</p><p><strong>Email:</strong> ${email}</p><p><strong>Temporary Password:</strong> ${tempPassword}</p><p>Please log in and change your password immediately.</p>`,
          }),
        }
      );
      if (!emailRes.ok) {
