...
const LineRow = ({
  ...
  <tr>
    ...
    <td style={{ ...cell, textAlign: 'left', whiteSpace: 'nowrap', color: tone ?? color.text }}>
...
const Email = ({
  ...
  >
    ...
    <EmailText>
      شكراً لاختيارك درب للدراسة في ألمانيا. تجد أدناه فاتورتك وملخص الدفعات الخاص بملفك.
    </EmailText>
    ...
    <Section style={metaBox}>
      <table width="100%" cellPadding={0} cellSpacing={0} role="presentation">
        <tbody>
          {invoiceNumber ? (
            <tr>
              <td style={metaLabel}>رقم الفاتورة</td>
              <td style={metaValue}>
          ...
          {issuedAt ? (
            <tr>
              <td style={metaLabel}>تاريخ الإصدار</td>
              <td style={metaValue}>
          ...
          {caseReference ? (
            <tr>
              <td style={metaLabel}>رقم الملف</td>
              <td style={metaValue}>
          ...
          {studentName ? (
            <tr>
              <td style={metaLabel}>الطالب</td>
              <td style={metaValue}>{studentName}</td>
            </tr>
          ) : null}
        </tbody>
      </table>
    ...
    {/* Services */}
    <Text style={sectionTitle}>خدمات درب (شيكل)</Text>
    <table width="100%" cellPadding={0} cellSpacing={0} role="presentation" style={lineTable}>
      <thead>
        <tr>
          <th style={{ ...headCell, textAlign: 'right' }}>الخدمة</th>
          <th style={{ ...headCell, textAlign: 'left' }}>المبلغ</th>
        </tr>
      </thead>
      <tbody>
        {services.map((s, i) => (
      ...
      <tfoot>
        ...
        {totalConfirmed ? (
          <LineRow label="المدفوع المؤكد" value={ils(totalConfirmed)} tone={color.success} />
        ) : null}
    ...
    {schoolCosts.length > 0 ? (
      <>
        <Text style={sectionTitle}>تكاليف ألمانيا (تقديرية — تُدفع مباشرة لمزوّد الخدمة)</Text>
        <table width="100%" cellPadding={0} cellSpacing={0} role="presentation" style={lineTable}>
          <tbody>
            {schoolCosts.map((l, i) => (
              <LineRow
    ...
    <EmailText muted>
      لأي استفسار بخصوص فاتورتك، يسعدنا تواصلك مع فريق درب.
    </EmailText>
  </EmailLayout>
)
